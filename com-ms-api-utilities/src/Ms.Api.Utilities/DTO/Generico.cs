...
namespace Ms.Api.Utilities.Contracts.DTOs
{
    public static class DynamicHelper
    {
        ...
        /// </summary>
        private static readonly Type[] _writeTypes = new[] { typeof(string), typeof(DateTime), typeof(Enumerable), typeof(decimal), typeof(Guid) };
        ...
        public static string ConvertToXml_v3(dynamic Object, Type type, string element = null)
        {
            string xml = null;
            using (System.IO.StringWriter sw = new System.IO.StringWriter())
            {
                ...
                if (!string.IsNullOrEmpty(element))
                    xs = new System.Xml.Serialization.XmlSerializer(type, new System.Xml.Serialization.XmlRootAttribute(element));
        ...
        public static string ToXML2(this object ojb, string root)
        {
            using (var stringwriter = new StringWriterUtf8())
            {
                var serializer = new System.Xml.Serialization.XmlSerializer(ojb.GetType(), new System.Xml.Serialization.XmlRootAttribute(root));
                serializer.Serialize(stringwriter, ojb, null);
        ...
        public static void ParseFromXML(dynamic parent, string xmlString)
        {
            XElement node = XElement.Parse(xmlString);
            ...
            if (node.HasElements)
            {
                if (node.Elements(node.Elements().First().Name.LocalName).Count() > 1)
                {
                    ...
                    {
                        ParseFromXML(list, element.ToString());
                    }
                ...
                else
                {
                    ...
                    foreach (var element in node.Elements())
                    {
                        ParseFromXML(item, element.ToString());
                    }
    ...
    public enum Atribute_Generic_Type
    {
        SKU_Caracteristica = 1,
        Produto_EspecificacaoTecnica = 2,
    }

    public class SkuImageProduct
    {
    ...
    public static class Utils
    {
        ...
        public static decimal ConvertInPoints(decimal valor, decimal fatorConversao)
        {
            return Math.Ceiling(valor / fatorConversao * 100) / 100;
        }

        public static decimal RemoveDecimalCases(this decimal value, bool retornarDecimal)
        {
