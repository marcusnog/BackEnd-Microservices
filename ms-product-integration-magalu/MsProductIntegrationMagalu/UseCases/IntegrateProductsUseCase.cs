...
using MsProductIntegrationMagalu.Extensions;
...
namespace MsProductIntegrationMagalu.UseCases
{
    public class IntegrateProductsUseCase : IIntegrateProductsUseCase
    {
        ...
        public readonly string _urlStoreDb;
            ...
            ILogger<IntegrateProductsUseCase> logger)
        {
            ...
            _urlProductsDb = String.Format(_configuration.GetValue<string>("Database_Products"), _store, _typeProductSku);
            _urlCategoryDb = _configuration.GetValue<string>("Database_Category");
            _urlStoreDb = _configuration.GetValue<string>("Database_Store");
        }
        ...
        public async Task IntegrateProducts()
        {
            try
            {
                ...
                var storeCodes = await GetStoreDB(_store);
                _logger.LogInformation($"Totals StoreCampaignCodes in returning: {storeCodes.Count()}");

                // load partner categories
                _logger.LogInformation("Getting categories from partner...");
                var partnerCategoriesAndProducts = await _integrationService.GetProductsAndCategories(storeCodes);
                _logger.LogInformation($"Totals categories in returning: {partnerCategoriesAndProducts.CategoriesPartner.Count()}");

                IEnumerable<ProductPartner> partnerProducts = null;

                // load partner products
                partnerProducts = partnerCategoriesAndProducts.ProductsPartner;
                ...
                foreach (var productPartner in partnerProducts)
                {
                    ...
                    var partnerCategory = partnerCategoriesAndProducts.CategoriesPartner.FirstOrDefault(c => c.DescCategoria == productPartner.DescCategoria);
                    if (partnerCategory == null) continue;

                    // map partner category to a valid category
                    var categoryDefault = MapCategory(categories, partnerCategory.DescCategoria);
                    if (categoryDefault == null) continue;

                    var ImagemProdutosUrl = productPartner.Images_list != null && productPartner.Images_list.Count > 0 ? productPartner.Images_list.FirstOrDefault().Imagem : string.IsNullOrEmpty(productPartner.ImagemProdutoDetalhe) ? (string.IsNullOrEmpty(productPartner.ImagemProdutoGrande) ? productPartner.ImagemVitrine : productPartner.ImagemProdutoGrande) : productPartner.ImagemProdutoDetalhe;
                    var _fotoPequena = productPartner.Images_list != null && productPartner.Images_list.Count > 0 ? productPartner.Images_list.FirstOrDefault().Imagem : ImagemProdutosUrl;
                    var _fotoMedia = productPartner.Images_list != null && productPartner.Images_list.Count > 0 ? productPartner.Images_list.FirstOrDefault().ImagemProdutoDetalhe : ImagemProdutosUrl;
                    var _fotoGrande = productPartner.Images_list != null && productPartner.Images_list.Count > 0 ? productPartner.Images_list.FirstOrDefault().ImagemProdutoDetalhe : ImagemProdutosUrl;
                    ...
                    {
                        _logger.LogInformation($"Sending creating product {productPartner.Descricao}");

                        // create product
                        await _sendMessageToQueueUseCase.Queue(new CreateProductSkuPreDefinedPriceEventV1()
                        {
                            Code = productCode,
                            DisplayName = productPartner.Descricao,
                    ...
                    {
                        _logger.LogInformation($"Sending updating product {productPartner.Descricao}");
                    ...
                    var partnerSkuCode = productCode + productPartner.Modelo;
                    ...
                    {
                        _logger.LogInformation($"Sending creating product sku {productPartner.Descricao}");
                        ...
                        await _sendMessageToQueueUseCase.Queue(new CreateSkuPreDefinedPriceEventV1()
                        {
                            ...
                            ListPrice = 0,
                            SalePrice = Convert.ToDecimal(productPartner.ValorVenda),
                            Availability = false,
                            Attributes = LstAttributesSku,
                            Model = productPartner.Modelo
                        });
                        continue;
                    }
                    ...
                    hash = JsonConvert.SerializeObject(new
                    {
                        Code = partnerSkuCode,
                        ListPrice = 0,
                        SalePrice = productPartner.ValorVenda,
                        Attributes = LstAttributesSku,
                        Tags = (string[]?)null,
                        Active = true,
                        Availability = false,
                    ...
                    _logger.LogInformation($"Sending updating product sku {productPartner.Descricao}");
                    // update sku
                ...
                _logger.LogInformation($"Time taken to import the store Magazine Luiza: {elapse.ToString(@"hh\:mm\:ss")} ");
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error when trying import products from store Magazine Luiza");
                throw ex;
            }
        }
        ...
        public async Task IntegratePrices()
        {
            try
            {
                ...
                var availabilityPartner = await _integrationService.GetAvailabilities(storeCodes);

                foreach (var availability in availabilityPartner)
                {
                    var produto = products.Values.Where(x => x.Skus.ContainsKey(availability.CodigoSKU.ToString())).FirstOrDefault();

                    if (produto == null) continue;

                    sku = produto.Skus[availability.CodigoSKU.ToString()];
            ...
            {
                _logger.LogError($"Error when trying import products availability from store Magazine Luiza");
                throw ex;
            }
        }
            ...
            try
            {
                ...
                foreach (var availability in availabilityPartner)
                {
                    ...
                    await _sendMessageToQueueUseCase.Queue(new AvailabilitySkuPreDefinedPriceEventV1()
                    {
                        SkuId = sku.Id,
                        ProductId = produto.Id,
                        Availability = Convert.ToBoolean(availability.Ativo)
                    });
                }
        ...
        async Task<IEnumerable<string>> GetStoreDB(string StoreId)
        {
            try
            {
                HttpClient client = new();

                var response = await client.GetAsync($"{_urlStoreDb}?storeId={StoreId}");
                var content = await response.Content.ReadAsStringAsync();

                if (!response.IsSuccessStatusCode)
                    throw new Exception("unexpected return", new Exception(content));

                var result = JsonConvert.DeserializeObject<DefaultResponse<Store>>(content);

                var storeCodesCampaignsMagalu = new List<string>();

                foreach (var item in result.Data.CampaignConfiguration)
                    storeCodesCampaignsMagalu.Add(item.StoreCode);

                return storeCodesCampaignsMagalu.ToArray();
            }
            catch (Exception ex)
            {
